...
def predict_cluster(df_new, # dataframe: trained clusters
    ...
    m = GridSearchCV(m, params,cv = 5) #will return warning if 'iid' param not set to true
...
def get_MDP(df_new):
    ...
    if 'End' in list(P_df['NEXT_CLUSTER'].unique()):
        ...
        end_clusters = P_df.loc[P_df['NEXT_CLUSTER']=='End']['CLUSTER'].unique()
        P_df = P_df.loc[P_df['NEXT_CLUSTER']!='End']
        
        # create dataframe that goal go to sink and sink go to sink
        s = max(P_df['CLUSTER'].max(), max(end_clusters)) + 1
        ...
        P_df = pd.concat([P_df, df_end]) # deprecated: P_df.append(df_end)
        P_df.sort_values(by=['CLUSTER','ACTION'], inplace=True)
        P_df.set_index(['CLUSTER','ACTION'], inplace=True)
        
        # set new reward node 
        R_df = pd.concat([R_df, pd.Series([0], index=[s])]) # deprecated: R_df.append(pd.Series([0], index=[s]))
...
def training_value_error(df_new, #Outpul of algorithm
    ...
    for i in range(N_train):
        ...
        while cont:
            ...
            # if df_new['RISK'].loc[index + t] > 0: 
            #     print('old ', df_new['RISK'].loc[index + t])
            #     print('real ', R_df.loc[s])

            # if df_new['RISK'].loc[index + t] !=  R_df.loc[s]: 
            #     print('s ', s)
            #     print('new ', df_new.loc[index + t])
...
def plot_features(df, x, y, c='CLUSTER', cmap='tab20'):
    fig, ax = plt.subplots()
    df.plot(kind="scatter", x=x, y=y, c=c, cmap=cmap, ax=ax, s=5)
...
def testing_accuracy(df_test, # dataframe: testing data
    ...
    print(model.cluster_map)
    print('clusters', clusters)
    print(test_clusters)
    print(df_test)
    print(clusters.loc[df_test['CLUSTER']].reset_index()['OG_CLUSTER'])
    # print(df_test)
    # print(clusters.index.unique())
    # print(clusters.loc[df_test['CLUSTER']])
