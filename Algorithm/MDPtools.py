...
#################################################################
def Generate_random_MDP(n,  # State space size
                        m,  # Nb of actions
...
def policy_value(mu, P, R, gamma=0.9, epsilon=10**(-10)):
    ...
    W = V.copy()
    for s in range(n):
        V[s] = R[s]+gamma*sum(mu[s, a]*sum(P[a, s, sp]*V[sp] for sp in range(n)) for a in range(m))
    while np.linalg.norm(V-W) > epsilon:
        W = V.copy()
        for s in range(n):
            V[s] = R[s]+gamma*sum(mu[s, a]*sum(P[a, s, sp]*V[sp] for sp in range(n)) for a in range(m))
...
def GetPolicy(V, P, R, gamma, prob='min'):
    if len(R.shape) == 2:
        R = expand(R)
    m, n, l = P.shape
    Vals = [[sum(P[a, i, :]*(R[a, i, :] + gamma*V)) for a in range(m)] for i in range(n)]
    if prob == 'min':
...
#################################################################
def SolveMDP(P,R, gamma=0.9, epsilon=10**(-10), p=False, prob='min', method='Value', threshold=float('inf')):
...
def LP_form(P, R, gamma=0.9):
    ...
    mdplp.addConstrs((J[i] <= sum([P[a,i,j]*(R[a,i,j]+gamma*J[j]) for j in range(n)]) for a in range(m) for i in range(n)))
...
def DualLP_form(P,R,gamma=0.9):
    ...
    for j in range(n):
        mdplp_dual.addConstr(sum(mu[j,:]) == c[j]+gamma*sum(sum(P[a,i,j]*mu[i,a] for i in range(n)) for a in range(m)))
    mdplp_dual.update()
    mdplp_dual.setObjective(sum(mu[i,a]*R[a,i,j]*P[a,i,j] for a in range(m) for i in range(n) for j in range(n)), GRB.MINIMIZE)
...
def sample_MDP_with_features(P,R, #MDP parameters
...
def sample_MDP_with_features_list(P,R, #MDP parameters
                             distributions,  #distribution[s] should be a function f, such that f() generates a radom element following this distribution
                             N,T  #sample size
                            ): 
    samples = []
...
def Random_Clustering(n,  #number of elements to cluster
...
def Construct_Clustered_MDP(P,R,delta):
    ...
    for a in range(m):
        for s in range(k):
            for sp in range(k):
                Pc[a,s,sp] = sum(sum(P[a,i,j]*delta[i,s]*delta[j,sp] for i in range(n)) for j in range(n)) / max(1,sum(delta[i,s] for i in range(n)))
                Rc[a,s] = sum(R[a,i]*delta[i,s] for i in range(n))/max(1,sum(delta[i,s] for i in range(n)))
    return Pc,Rc
...
def accuracy_clustering_policy(delta,pi):
    ...
    for s in range(k):
        ...
        acc[s]=[int(100*sum(acc[s] == a)/max(1,len(acc[s])))/100 for a in range(m)]
...
def Data_Random_Clustering(N,T,  #number of elements to cluster
                      k):  #number of clusters
    #Generating random clustering
...
def Data_Construct_Clustered_MDP(samples,m,delta):
    ...
    for a in range(m):
        for s in range(k):
            for sp in range(k):
                Pc[a,s,sp] = sum(delta[u[0],u[1],s]*delta[u[0],u[1]+1,sp] for u in samples_action[a] if u[1]<T-1)/max(1,sum(delta[u[0],u[1],s] for u in samples_action[a] if u[1]<T-1))
            Rc[s] = sum(sum(samples[i,t][2]*delta[i,t,s] for i in range(N)) for t in range(T))/sum(sum(delta[i,t,s] for i in range(N)) for t in range(T))
    return Pc,Rc
...
def Data_action_Cluster_size(samples,delta,m):
    N,T,k = delta.shape
    samples_action = select_action(samples,m)
    return np.array([[sum(delta[u[0],u[1],s] for u in samples_action[a] if u[1]<T-1) for s in range(k)] for a in range(m)])
...
def truth_based_clustering(samples,k,outcome_dist=norm2_dist,transition_dist=norm2_dist):
    ...
    outcome_loss_s,transition_loss_s = outcome_pred_loss(samples,delta_s,cluster_of_s,outcome_dist,Rc_s),transition_pred_loss(samples,delta_s,cluster_distance_s,cluster_of_s,Pc_s)
    c_a_s = Data_action_Cluster_size(samples,delta_s,m)
    return delta_s,cluster_of_s,Pc_s,Rc_s,Vc_s,outcome_loss_s,transition_loss_s,c_s,c_a_s


def clustering_charachteristics(samples,delta,outcome_dist=norm2_dist,transition_dist=norm2_dist):
    k = delta.shape[2]
    ...
    outcome_loss,transition_loss = outcome_pred_loss(samples,delta,cluster_of,outcome_dist,Rc),transition_pred_loss(samples,delta,cluster_distance,cluster_of,Pc)
    c_a = Data_action_Cluster_size(samples,delta,m)
    return cluster_of,Pc,Rc,Vc,outcome_loss,transition_loss,cluster_distance,c,c_a
